...
async function initNormalMap() {
    ...
    google.maps.event.addDomListener(window, 'load', () => {
        initializePostalCodeOverlay();  // Define after Maps library is ready
    });
...
function handleBothClicks(e) {
    ...
    if (isRecruitmentRegion(placeIdBoth)) {
        ...
        const userChoice = confirm("Add this region to Area Development (OK) or Recruitment (Cancel)?");
        if (userChoice) {
            handleAreaDevClick(e);
        } else {
            handleRecruitmentClick(e);
...
function handleClick(e) {
    if (isAreaDevClicked && isRecruitmentClicked) {
        ...
        handleAreaDevClick(e); // Only Area Development is active
    } else if (isRecruitmentClicked) {
        handleRecruitmentClick(e); // Only Recruitment is active
    }
}
...
function get_all_place_ids(bounds){
    var all_place_ids = []
    featureLayer.style = (params) => {
        const feature = params.feature;
        if (feature) {
...
function load_postal_data_Recruitment(data_list_recruit){
    $.ajax({
            ...
            success: function(response) {
                ...
                try{
                   createLabelOverlayRecruitment( JSON.parse(latLng),postal_no)
                   }
                catch{console.log('..')}
                  }
...
function get_all_place_ids_Recruitment(){
    var get_all_place_ids_Recruitment = []
    featureLayer.style = (params) => {
        const feature = params.feature;
        if (feature) {
            const placeId = feature.Hg;
            get_all_place_ids_Recruitment.push(placeId)
        }
    return applyStyle(params);
    };
        setTimeout(
              function()
              {
                load_postal_data_Recruitment(get_all_place_ids_Recruitment)
                return get_all_place_ids_Recruitment
              }, 1000);
}
...
function setupMapClickListener() {
    google.maps.event.addListener(map, 'click', function(event) {
        const table = document.getElementById("demographic-table-radial");
        if (table && table.style.display === "block" && selectedCircleId) {
            table.style.display = "none";
            resetCircleHighlight(selectedCircleId);
            selectedCircleId = null;
        }
...
async function fetchPopulationData(center, radius) {
    try {
        // Make sure center is a proper object with lat and lng properties
...
async function fetchDataByPostalCode(postalCode, isFirstZipCode = false, populationValue = null) {
    try {
        ...
        const result = await response.json();
        if (result.success) {
            // Only update the population if this is the first zip code and we have a population value
            if (isFirstZipCode && populationValue !== null) {
                result.data.population = populationValue;
            } else if (!isFirstZipCode && populationValue !== null) {
                // For subsequent zip codes, set population to 0 to avoid double counting
                result.data.population = 0;
            }
            
            RadialpopulateTable(result.data);
...
const closeModalPostal = document.getElementById('closeModalPostal');
const postalCodesList = document.getElementById('postalCodesList');
closeModalPostal.onclick = function() {
    postalCodeModal.style.display = "none";
}
window.onclick = function(event) {
    if (event.target == postalCodeModal) {
    postalCodeModal.style.display = "none";
    }
}
function showPostalCodes(postalCodes) {
    postalCodesList.innerHTML = ''; 
    if (postalCodes.length > 0) {
        postalCodes.forEach(code => {
            const div = document.createElement('div');
            div.className = 'postal-code-item';
            div.textContent = code;
            postalCodesList.appendChild(div);
        });
    } else {
        postalCodesList.innerHTML = '<div class="postal-code-item">No postal codes found</div>';
    }
...
async function getPlacesInCircle(center, radius) {
    ...
    try {
        ...
        // showPostalCodes(uniquePostalCodes);
        for (let i = 0; i < uniquePostalCodes.length; i++) {
            const postalCode = uniquePostalCodes[i];
            const isFirst = i === 0; 
            await fetchDataByPostalCode(postalCode, isFirst, populationData);
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    console.error("Error saving circle data:", response.message);
